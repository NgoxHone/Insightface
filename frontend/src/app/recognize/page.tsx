...
export default function RecognizePage() {
  ...
  const handleFileSelect = useCallback(
    (file: File) => {
      ...
      // Clear canvas to remove old drawings
    ...
    faces.forEach((face, idx) => {
      try {
        ...
        const paddedWidth = Math.min(
          img.naturalWidth - paddedX1,
          width * (1 + 2 * padding),
        );
        const paddedHeight = Math.min(
          img.naturalHeight - paddedY1,
          height * (1 + 2 * padding),
        );
  ...
  const handleAddAndTrain = async (idx: number, name: string) => {
    const targetName = name.trim();
    if (!targetName) {
      setRowMessages((prev) => ({
        ...prev,
        [idx]: { type: "error", text: "Vui lòng nhập tên người dùng" },
      }));
      return;
    }

    const faceData = faceImages[idx];
    if (!faceData) {
      setRowMessages((prev) => ({
        ...prev,
        [idx]: { type: "error", text: "Không lấy được ảnh khuôn mặt" },
      }));
      return;
    }
    ...
    setRowMessages((prev) => ({
      ...prev,
      [idx]: { type: "success", text: "Đang thêm ảnh và train..." },
    }));

    try {
      const faceFile = dataUrlToFile(
        faceData,
        `${targetName}_${Date.now()}.jpg`,
      );
      const uploadRes = await uploadPersonImages(targetName, [faceFile]);
      if (!uploadRes.success) {
        throw new Error(uploadRes.error || "Không thể thêm ảnh");
      }

      const trainRes = await trainPerson(targetName);
      if (!trainRes.success) {
        throw new Error(trainRes.error || "Không thể train");
      }

      setRowMessages((prev) => ({
        ...prev,
        [idx]: { type: "success", text: `Đã thêm ảnh và train ${targetName}` },
      }));
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Lỗi khi thêm ảnh và train";
      setRowMessages((prev) => ({
        ...prev,
        [idx]: { type: "error", text: message },
      }));
    } finally {
  ...
  const handleAppendAndRetrain = async (idx: number, face: FaceResult) => {
    if (!face.matched || face.name === "unknown") {
      setRowMessages((prev) => ({
        ...prev,
        [idx]: {
          type: "error",
          text: "Không có người dùng phù hợp để cập nhật",
        },
      }));
      return;
    }

    await handleAddAndTrain(idx, face.name);
  };
  ...
  return (
    <div className="animate-fade-in">
      ...
      <p className="text-gray-400 mb-6">
        Upload ảnh để phát hiện và nhận diện khuôn mặt
      </p>
                ...
                <tbody className="divide-y divide-[#2a2a2a]">
                  {results.map((face, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`font-semibold ${face.matched ? "text-[#03dac6]" : "text-[#cf6679]"}`}
                        >
                          {face.name === "unknown"
                            ? "Không xác định"
                            : face.name}
                      ...
                      <td className="px-4 py-3">
                        ...
                        >
                          <FontAwesomeIcon
                            icon={
                              face.matched ? faCheckCircle : faExclamationCircle
                            }
                            className="text-xs"
                          />
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-2 min-w-[260px]">
                            ...
                            <>
                              ...
                              <div className="flex items-center gap-2">
                                <input
                                  ...
                                  placeholder="Nhập tên user mới / hiện có"
                                  className="bg-[#252525] border border-[#444] rounded px-2 py-1.5 text-sm text-[#ededed] w-full"
                                />
                                <button
                                  onClick={() =>
                                    handleAddAndTrain(
                                  ...
                                  className="bg-[#03dac6]/10 border border-[#03dac6]/30 text-[#03dac6] px-3 py-1.5 rounded text-xs whitespace-nowrap disabled:opacity-50"
                                >
                                  {rowLoading[idx]
                                    ? "Đang train..."
                                    : "Thêm & Train"}
                          ...
                          {face.confidence > 0.25 && (
                            <button
                              onClick={() => handleAppendAndRetrain(idx, face)}
                              disabled={
                                !!rowLoading[idx] ||
                                !face.matched ||
                                face.name === "unknown"
                              }
                              className="bg-[#bb86fc]/10 border border-[#bb86fc]/30 text-[#bb86fc] px-3 py-1.5 rounded text-xs text-left disabled:opacity-50"
                            >
                              {rowLoading[idx]
                                ? "Đang train..."
                                : "Thêm vào tập ảnh sẵn có & Train lại"}
                            </button>
                          )}

                          {!(
                            face.confidence < 0.25 || face.confidence > 0.65
                          ) && <span className="text-xs text-gray-500"></span>}

                          {rowMessages[idx] && (
                            <span
                              className={`text-xs ${rowMessages[idx].type === "success" ? "text-[#03dac6]" : "text-[#cf6679]"}`}
                            >
                              {rowMessages[idx].text}
                            </span>
                          )}
                        </div>
                      </td>
